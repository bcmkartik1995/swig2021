...
switch($accessCase)
{
	case 'GET':
		...
		{
			$usersInfoArr = $objDBQuery->getRecord(0, $arrFld, $tblUsers, '', '', '', 'createdOn', 'ASC');
			if (is_array($usersInfoArr)) $rtnRes = array('status' => 1, 'msg' => 'all users data has been retrieved successfully.', 'totalRecords' => count($usersInfoArr), 'data' => array());
			else $rtnRes = array('status' => 0, 'msg' => "Sorry, no data found.");		
		}
		responses(0, $rtnRes);
		break;
	...
	case 'forgotPassword':		
		...
		{				
			...
			if ($objDBQuery->updateRecord(0, $dataArr, $tblUsers, $whereArr))
			{
				...
				$emailFormatInfoArr = $objDBQuery->getEmailFormat(0, 'app-forgot-password');
	...
	case 'validateAccountActivationCode':		
		...
		else $rtnRes = array('status' => 0, 'msg' => "Sorry, your account activation code does not match in our database. Please genrate new account activation code by clicking resend link");	
		responses(0, $rtnRes);
		break;
	...
	case 'resendAccountActivationCode':		
		...
		{			
			...
			if ($objDBQuery->updateRecord(0, $dataArr, $tblUsers, $whereArr))
			{
				...
				$emailFormatInfoArr = $objDBQuery->getEmailFormat(0, 'app-resend-account-activation-code');
	...
	case 'checkUserLogin':		
		...
		else if ($objDBQuery->getRecordCount(0, $tblUsers, array($dbFld => $email, 'password' => generatePassword($password), 'accountStatus' => 'I')))
		{
			$rtnRes = array('status' => 0, 'msg' => 'Your account has been suspended, please contact to help desk.');
		}
		else if ($objDBQuery->getRecordCount(0, $tblUsers, array($dbFld => $email, 'password' => generatePassword($password), 'accountStatus' => 'N')))
		{
		...
		else if ($objDBQuery->getRecordCount(0, $tblUsers, array($dbFld => $email, 'password' => generatePassword($password))))
		{	
			$dataArr['updatedOn'] = date(LONG_MYSQL_DATE_FORMAT);			
			
			$usersInfoArr = $objDBQuery->getRecord(0, $arrFld, $tblUsers, array($dbFld => $email, 'password' => generatePassword($password)));
	...
	case 'register':
		...
		else if (filter_var($email, FILTER_VALIDATE_EMAIL))
		{
			$userCode = randomMD5(); //
			$accountActivationCode = makeRandNo6Digit();
			$frmKeyExcludeArr = array('newPinRecovery', 'confirmPassword', 'postAction', 'appId');		
			...
			{				
				...
				$emailFormatInfoArr = $objDBQuery->getEmailFormat(0, 'app-signup');
		...
		else if ($objDBQuery->getRecordCount(0, $tblUsers, $whereArr))
		{
			...
			else if ($objDBQuery->updateRecord(0, $dataArr, $tblUsers, $whereArr))
			{
				if (strtolower($email) !=  strtolower($oldEmail))
				{
					...
					$emailFormatInfoArr = $objDBQuery->getEmailFormat(0, 'app-update-email');
				...
				if (generatePassword($password) !=  $oldPassword)
				{
					...
					$emailFormatInfoArr = $objDBQuery->getEmailFormat(0, 'app-change-password');
