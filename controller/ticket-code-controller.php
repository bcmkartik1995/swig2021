...
$tblName = 'tbl_ticket_codes';
$enckeyDBFldName = 'ticketId_PK';
$msgTxt = 'Subcategory';
...
switch ($accessCase) 
{
	...
	case 'addAction':		
		...
		if ($objDBQuery->getRecordCount(0, $tblName, "subCatName = '".strtolower($dataArr['subCatName'])."'"))
		{
			viewState($dataArr, 1);
	...
	case 'generateTicketCodes':		
		$_POST = trimFormValue(0, $_POST);
		$headerRedirectUrl = '../generate-ticket-codes.php?'.$_SESSION['SESSION_QRY_STRING_FOR_SUB_CATE'];;
		$frmKeyExcludeArr = array('submitBtn', 'formToken', 'postAction', 'enkey', 'photo_delete');
		$dataArrs = prepareKeyValue4Msql(0, $_POST, $frmKeyExcludeArr);
		$noOfTicket = $dataArrs['noOfTicket'];

		if (!validateForm($_SESSION['formValidation']) && is_numeric($noOfTicket))
		{
			$msg = '';
			...
			$msg = "Ticket codes have been generated successfully.";				
			$_SESSION['msgTrue'] = 1;
			$headerRedirectUrl = '../view-all-ticket-codes.php?'.$_SESSION['SESSION_QRY_STRING_FOR_SUB_CATE'];	
			$_SESSION['messageSession'] = $msg;	
		}
		else
		{
			viewState($dataArr, 1);
			$_SESSION['messageSession'] = "Please enter No Of Tickets Number";	
		}
		//print_r(validateForm($_SESSION['formValidation']));
	...
	case 'updateAction':
		...
		if ($objDBQuery->getRecordCount(0, $tblName, "$enckeyDBFldName != '$enkey' AND subCatName = '".strtolower($menuName)."'"))
		{
