...
else if ($postAction != '' && $appCode != '')
{
	...
	if (!empty($appInfoArray) && is_array($appInfoArray))
	{
		...
		$arrSelectDbFlds4Stream = array('m.menuType', 'menuCode_FK', 's.streamCode', 's.streamTitle', 's.streamUrl', 's.streamImg', 's.streamDuration','s.createdOn', 's.updatedOn', 's.subCatCode_FK');
		...
		{					
			...
			{						
				...
				$subCatCode_FK = $appStreamsInfoArr[$j]['subCatCode_FK'];
