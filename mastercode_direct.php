...
if($postAction == "validateGuest")
{
	...
	if($guestEmail != "")
	{
			...
			if(!$objDBQuery->getRecordCount(0, 'tbl_guest_directaccess', array('appCode_FK' => $appCode, 'guest_email' => $guestEmail)))
				$rtnRes = array('status' => 0, 'msg' => 'Sorry, we do not have your e-mail listed on the Guest list.');
			else
			{				
...
else if($postAction == "validateMasterCode")
{
	$arrInsert = array('ipAddress' => $ipAdd, 'appCode_FK' => $app_code,'ticket_code_type' => 'M', 'cust_email_address' => $_REQUEST['emailAddress'], 'guest_ticket_code' => $_REQUEST['ticketCode']);
		
	if($_REQUEST['ticketCode'] != "")
	{
			//$_POST = trimFormValue(0, $_POST);
			$appCode = $_REQUEST['appId'];
			$ticketCode = strtoupper($_REQUEST['ticketCode']);		
		...
		//	echo "app:".$appCode;
		//	echo "\ncode=".$ticketCode;
		//	echo "\n".$objDBQuery->getRecordCount(0, 'tbl_master_codes', array('appCode_FK' => $appCode, 'masterCode' => $ticketCode));
		
			if(!$objDBQuery->getRecordCount(0, 'tbl_guest_directaccess', array('appCode_FK' => $appCode, 'guest_email' => $guestEmail)))
				$rtnRes = array('status' => 0, 'msg' => 'Sorry, we do not have your e-mail listed on the Guest list.');
			else if (!$objDBQuery->getRecordCount(0, 'tbl_master_codes', array('appCode_FK' => $appCode, 'masterCode' => $ticketCode))) 
				$rtnRes = array('status' => 0, 'msg' => 'Ticket Code does not match with our system.');
			else if ($appCode != '' && $ticketCode != '')
			{				
				$dataArr4Ticket = array('isUsed' => 'Y', 'updatedOn' => date(LONG_MYSQL_DATE_FORMAT));
				$objDBQuery->addRecord(0, $arrInsert, 'tbl_track_users');
