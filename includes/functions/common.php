...
{
	date_default_timezone_set("UTC");
	$readableCurrDt = date('Y-m-d H:i:s');	
	$readableExtendedDt = date(LONG_MYSQL_DATE_FORMAT, strtotime("$interval", strtotime($readableCurrDt)));	
...
function sendEmail($to, $subject, $body, $fromName, $from, $format = '')
{
	...
	//$subject = str_replace(array_keys($ARR_CHARS_STR), array_values($ARR_CHARS_STR), $subject);
	//$body = str_replace(array_keys($ARR_CHARS_STR), array_values($ARR_CHARS_STR), $body);
	...
	{
		if (php_uname("n") == 'ip-172-31-48-252' ||  php_uname("n") == 'r410-1' || php_uname("n") == 'webserver'  || php_uname("n") == 'gator4172.hostgator.com')
		{
			$success = sendEmailUsingSES($to, $subject, $body, $fromName, $from);			
		}
		else 
		{
...
function prepareEmailFormat($traceArr, $emailFormatInfoArr, $objDBQuery, $appCode, $fname = '', $emailORUsername = '', $password = '', $accountActivationCode = '', $eventOrStreamTitle = '', $ticketCode = '', $timing = '', $time = '')
{
	...
	$emailSubject = str_replace(array('[TICKET_CODE]'), array($ticketCode), $emailSubject); 
...
function convertDateTimeSpecificTimeZone($dt, $tz1 = 'America/New_York', $df = 'Y-m-d H:i:s', $tz2 = 'UTC')
{
 ...
 function generateUniqueCode($excludeCodesArray = '', $codeLength = 10)
 {
	//$characters = "WXYZ23456@7819ABCD#EFGH&JKL$MNPQRS*TUV";
	$characters = "WXYZ23456@7819ABCD#EFGHJKL$MNPQRS*TUV";
	$code = "";
	...
	for ($j = 0; $j < 1; $j++)
	{
		...
		if (@is_array($excludeCodesArray))
		{
			if (!@in_array($code, $excludeCodesArray)) $uniqueCode = $code;
			else $j--;
		}
		else
		{
